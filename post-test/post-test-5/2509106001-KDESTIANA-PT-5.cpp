...
void selectionSortHargaDescending(DataWisata arr[], int n) {
    for (int i = 0; i < n - 1; i++) {
        int indeksMax = i;
        for (int j = i + 1; j < n; j++) {
            if (arr[j].hargaTiket > arr[indeksMax].hargaTiket) {
                indeksMax = j;
            }
        }
        if (indeksMax != i) {
...
void menuSorting(DataWisata *daftarAsli, int count) {
    DataWisata salinan[MAX_DATA];
    for (int i = 0; i < count; i++) salinan[i] = daftarAsli[i];

    int pil;
    cout << "\n--- Pilih Urutan Tampil ---" << endl;
    cout << "1. Nama A-Z          (Bubble Sort)"  << endl;
    cout << "2. Harga Tertinggi   (Selection Sort)"  << endl;
    cout << "3. Kuota Terendah    (Insertion Sort5)"     << endl;
    cout << "0. Tanpa pengurutan" << endl;
    cout << "Pilihan: "; cin >> pil;

    switch (pil) {
        case 1:
            bubbleSortNamaAscending(salinan, count);
            cout << "\n=== KATALOG WISATA - Nama A-Z (Bubble Sort) ===" << endl;
            break;
        case 2:
            selectionSortHargaDescending(salinan, count);
            cout << "\n=== KATALOG WISATA - Harga Tertinggi (Selection Sort) ===" << endl;
            break;
        case 3:
            insertionSortKuotaAscending(salinan, count);
            cout << "\n=== KATALOG WISATA - Kuota Terendah (Insertion Sort) ===" << endl;
            break;
        default:
            cout << "\n=== KATALOG WISATA ===" << endl;
            break;
...
void lihatDataWisata(DataWisata *arr, int count) {
    cout << "\n=== LIHAT DESTINASI WISATA ===" << endl;
    if (count == 0) { cout << "Belum ada data." << endl; return; }
    menuSorting(arr, count);
}
...
bool prosesLoginRekursif(AkunUser *dbUser, int *jumlahUser, int sisaPercobaan) {
    ...
    cout << "\n=== SILAHKAN LOGIN (Percobaan "
         << (BATAS_LOGIN - sisaPercobaan + 1) << "/" << BATAS_LOGIN << ") ===" << endl;
...
void menuUtamaCRUD(DataWisata *daftarWisata, int *jumlahData) {
    ...
    do {
        ...
        cout << "2. Lihat Destinasi (Sorting)"  << endl;
